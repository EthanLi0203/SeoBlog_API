...
exports.preSignup = (req, res) => {
    ...
    User.findOne({email: email.toLowerCase()}).exec((err, user) => {
        ...
        sgMail.send(emailData).then(sent => {
            return res.json({
                message: `Email has been sent to ${email}. Please following the instructions to activate your account.`
            })
        })
    })  
}
        ...
        jwt.verify(resetPasswordLink, process.env.JWT_RESET_PASSWORD, function(err, decoded) {
            ...
            User.findOne({ resetPasswordLink }, (err, user) => {
                if (err || !user) {
                    console.log(err)
                    return res.status(401).json({
                        error: 'Using the latest link or trying to send reset password again'
                    });
                }
                const updatedFields = {
